...
def extract_words(text):
    ...
    for char in text:
        ...
        if char in ascii_letters:
            ...
            # add word to words if word isn't empty
            if word:
                words.append(word)
            word = ""
...
def analyze_tweet_sentiment(tweet):
    ...
    # create list of sentiments of words
    sentiments = []
    for word in words:
    ...
    # handle case where number of words with sentiment is 0
    if len(sentiments) == 0:
...
def find_centroid(polygon):
    ...
    # handle case where area = 0
    if signed_area == 0:
        return (latitude(polygon[0]), longitude(polygon[0]), 0)

    # compute centroid using formula
    sum_lat = 0
    sum_lon = 0
    ...
    for i in range(n):
        ...
        term = (lat1*lon2 - lat2*lon1)
        sum_lat += (lat1 + lat2) * term
        sum_lon += (lon1 + lon2) * term

    lat_average = sum_lat / (6*signed_area)
    lon_average = sum_lon / (6*signed_area)
    
    return (lat_average, lon_average, abs(signed_area))
...
def find_state_center(polygons):
    ...
    for polygon in polygons:
        # get area and centroid of polygon
        result = find_centroid(polygon)
        area = result[2]
        centroid = make_position(result[0], result[1])
        
        areas.append(area)
        centroids.append(centroid)

        # multiply centroid by area
        lat = latitude(centroid)
        lon = longitude(centroid)
        lat_products.append(lat * area)
        lon_products.append(lon * area)

    # take the sum and divide by total area to get weighted average
    total_lat = sum(lat_products)
    total_lon = sum(lon_products)
    
    total_area = sum(areas)
    center = make_position(total_lat/total_area, total_lon/total_area)
    return center    
...
def group_tweets_by_state(tweets):
    ...
    """
    # get dictionary with states and their centers
    state_centers = {}
    for state in us_states.keys():
        polygons = us_states[state]
        center = find_state_center(polygons)
        state_centers[state] = center

    # add each tweet and closest state to new dictionary
    tweets_by_state = {}
    for tweet in tweets:
        position = tweet_location(tweet)

        # use distance of state from position as key for min
        def distance(state):
            center = state_centers.get(state)
            return geo_distance(position, center)

        # put closest state and tweet in dictionary
        closest_state = min(state_centers.keys(), key=distance)
        if tweets_by_state.get(closest_state):
...
def average_sentiments(tweets_by_state):
    ...
    # for each state make a list of its tweets' sentiments
    for state in tweets_by_state.keys():
        ...
        for tweet in tweets_by_state[state]:
            ...
            if has_sentiment(sentiment):
                sentiment_val = sentiment_value(sentiment)
                sentiments.append(sentiment_val)
        # find average of list of sentiments and add it to dictionary
        if sentiments:

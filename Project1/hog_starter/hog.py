...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        rolls.append(dice())
    # if theres a 1, return 1. else return the sum
    if rolls.count(1) != 0:
        return 1
    sum = 0
    for roll in rolls:
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # implement free bacon if num_rolls is 0
    if num_rolls == 0:
        a = opponent_score % 10
        b = opponent_score // 10
        return 1 + max(a, b)
    # otherwise score is the result of rolling dice
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < 100 and opponent_score < 100:
        ...
        dice = select_dice(score, opponent_score)
        roll = take_turn(num_rolls, score, dice)
        opponent_score += roll
        if score*2 == opponent_score or opponent_score*2 == score:
            score, opponent_score = opponent_score, score
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    scores = []
    for i in range(1, 11):
        avg_score = average_dice(i, dice)
        scores.append(avg_score)
        print(f"{i} dice scores {avg_score} on average")
...
def swap_strategy(score, opponent_score):
    ...
    points = max(a, b) + 1
    score0 = score + points
    if score0*2 == opponent_score or score0 == opponent_score*2:
        if score0 < opponent_score:
            return 0
        else:
            return BASELINE_NUM_ROLLS
    else:
...
def final_strategy(score, opponent_score):
    """
    Description:
    1. See if rolling 0 will make you win by making sure the score is >= 100 and that it isn't double the opponent's score
    2. Try to force the opponent to roll 4 dice by lowering the threshold for bacon_strategy
    3. Otherwise use swap strategy
    """
    a = opponent_score // 10
    b = opponent_score % 10
    points = max(a, b) + 1
    ...
    if score0 >= 100:
        if opponent_score*2 != score0:
            return 0
    
    if (score0 + opponent_score) % 7 == 0 and points > 6:
        return 0
